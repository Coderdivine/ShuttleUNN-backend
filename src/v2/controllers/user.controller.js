...
class UserContoller {
  ...
  async registerNotifcationDevice(req, res) {
    ...
    const result = await UserService.registerNotifcationDevice(user_id, {
      ...req.body,
      device_name,
    });
    res
      ...
      .send(
        response(
          "Device registered. Start recieving notifcation from DevSensor",
          result
        )
      );
  }
  ...
  async user_data(req, res) {
    ...
    if (!result)
      throw new CustomError("Oops! unable to get user data failed", 500);
  ...
  async resetPassword(req, res) {
    ...
    res
      ...
      .send(
        response(
          "OTP sent. Please check your email for the one-time password (OTP).",
          result
        )
      );
  }
  ...
  async suspendAccount(req, res) {
    if (!req.body)
      throw new CustomError(
        "The application is experiencing problems with sending your request. Please attempt again later or log in.",
        400
      );
  ...
  async registerDeviceForNotification(req, res) {
    if (!req.body)
      throw new CustomError(
        "There are issues with processing your request. Please try again later or log in.",
        400
      );
    const result = await UserService.registerDeviceForNotification(
      req.body.user_id,
      req.body
    );
    if (!result)
    ...
    res
      .status(201)
      .send(response("Device registered for notifications.", result));
  }
  ...
  async deleteRegisteredDevice(req, res) {
    ...
    const result = await UserService.deleteRegisteredDevice(
      req.body.user_id,
      req.body
    );
    if (!result)
  ...
  async addToUserbilling(req, res) {
    ...
    const result = await UserService.addToUserbilling(
      req.body.user_id,
      req.body
    );
    if (!result)
  ...
  async addToUserRoutine(req, res) {
    ...
    const result = await UserService.addToUserRoutine(
      req.body.user_id,
  ...
  async editUserRoutine(req, res) {
    ...
    const result = await UserService.editUserRoutine(
      req.body.user_id,
      req.body.routine_id,
      req.body.updatedValue,
      req.body.updatedField
    );
    if (!result)
      throw new CustomError(
        "Something went wrong. please try again later",
  ...
  async deleteFromUserRoutine(req, res) {
    ...
    const result = await UserService.deleteFromUserRoutine(
      req.body.user_id,
      req.body.routine_id
    );
    if (!result)
  ...
  async useProfileForWorkout(req, res) {
    ...
    res
      ...
      .send(
        response(
          `profile will ${value ? " be used " : " not be used "} for excerise.`,
          result
        )
      );
  }

  async useMagicFix(req, res) {
    const result = await UserService.useMagicFix(req.params.user_id);
    if (!result)
      throw new CustomError(
        "Something went wrong. please try again later",
        400
      ...
      console.log({ result });
      const magicFix = result?.magicFix;
    res
      .status(201)
      .send(
        response(
          `Magic Fix ${magicFix ? "Enabled": "Disabled" }.`,
          result
        )
      );
  }

  async magicFixEnabled(req, res) {
    const result = await UserService.magicFixEnabled(req.params.user_id);
    if (!result)
      throw new CustomError(
        "Something went wrong. please try again later",
        400
